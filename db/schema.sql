...
--     alert_days_before INTEGER DEFAULT 30, -- Alert when X days before expiry
--     last_checked TIMESTAMP,
--     check_interval_hours INTEGER DEFAULT 24, -- Check every 24 hours
    
--     -- Status
--     domain_status VARCHAR(50), -- 'active', 'expiring_soon', 'expired', 'error'
...
--     days_until_expiry INTEGER,
--     registrar VARCHAR(255),
--     status VARCHAR(50), -- 'success', 'failed', 'timeout', 'dns_error'
...
--     alert_type VARCHAR(50) NOT NULL, -- 'expiring_soon', 'expired', 'check_failed', 'renewed'
...
--     check_interval INTEGER NOT NULL DEFAULT 3600, -- 1 hour default
--     warning_days INTEGER NOT NULL DEFAULT 30,     -- warn if expiring within 30 days
--     critical_days INTEGER NOT NULL DEFAULT 7,     -- critical if expiring within 7 days
...
-- -- INDEXES
-- -- ============================================
-- CREATE INDEX idx_monitored_domains_enabled ON monitored_domains(monitoring_enabled);
-- CREATE INDEX idx_monitored_domains_expiry ON monitored_domains(expiry_date);
-- CREATE INDEX idx_monitored_domains_status ON monitored_domains(domain_status);
-- CREATE INDEX idx_monitored_domains_last_checked ON monitored_domains(last_checked);

-- CREATE INDEX idx_domain_check_history_domain ON domain_check_history(domain_id);
-- CREATE INDEX idx_domain_check_history_checked ON domain_check_history(checked_at);

-- CREATE INDEX idx_domain_alerts_domain ON domain_alerts(domain_id);
-- CREATE INDEX idx_domain_alerts_notified ON domain_alerts(notified);
-- CREATE INDEX idx_domain_alerts_created ON domain_alerts(created_at);
...
-- CREATE OR REPLACE FUNCTION update_domain_expiry_status()
-- RETURNS TRIGGER AS $$
-- BEGIN
--     -- Calculate days until expiry
--     NEW.days_until_expiry := EXTRACT(DAY FROM (NEW.expiry_date - NOW()))::INTEGER;
    
--     -- Update status based on days remaining
--     IF NEW.expiry_date < NOW() THEN
--         NEW.domain_status := 'expired';
--     ELSIF NEW.days_until_expiry <= NEW.alert_days_before THEN
--         NEW.domain_status := 'expiring_soon';
--     ELSE
--         NEW.domain_status := 'active';
--     END IF;
...
-- CREATE TRIGGER trigger_update_domain_expiry_status
--     BEFORE INSERT OR UPDATE OF expiry_date, alert_days_before ON monitored_domains
--     FOR EACH ROW EXECUTE FUNCTION update_domain_expiry_status();

-- -- Notification trigger for domain alerts
-- CREATE OR REPLACE FUNCTION notify_domain_alert()
-- RETURNS TRIGGER AS $$
-- DECLARE
--     notification JSON;
--     domain_info RECORD;
-- BEGIN
--     -- Get domain info
--     SELECT domain_name, expiry_date, days_until_expiry, domain_status
--     INTO domain_info
...
--         'expiry_date', to_char(domain_info.expiry_date, 'YYYY-MM-DD'),
--         'domain_status', domain_info.domain_status,
--         'created_at', to_char(NEW.created_at AT TIME ZONE 'UTC', 'YYYY-MM-DD"T"HH24:MI:SS.US')
--     );
    
--     PERFORM pg_notify('domain_alerts', notification::text);
    
--     RETURN NEW;
-- END;
-- $$ LANGUAGE plpgsql;

-- CREATE OR REPLACE FUNCTION get_expiring_domains(days_threshold INTEGER DEFAULT 30)
-- RETURNS TABLE(
--     id INTEGER,
...
--       AND d.days_until_expiry <= days_threshold
--       AND d.days_until_expiry >= 0
...
--         d.domain_name,
--         d.expiry_date,
--         ABS(d.days_until_expiry) as days_since_expiry,
--         d.registrar
--     FROM monitored_domains d
--     WHERE d.monitoring_enabled = TRUE
...
-- CREATE OR REPLACE FUNCTION update_all_domain_statuses()
-- RETURNS INTEGER AS $$
-- DECLARE
--     affected_rows INTEGER;
-- BEGIN
--     UPDATE monitored_domains
--     SET days_until_expiry = EXTRACT(DAY FROM (expiry_date - NOW()))::INTEGER,
--         domain_status = CASE 
--             WHEN expiry_date < NOW() THEN 'expired'
--             WHEN EXTRACT(DAY FROM (expiry_date - NOW()))::INTEGER <= alert_days_before THEN 'expiring_soon'
--             ELSE 'active'
--         END,
--         updated_at = NOW()
...
-- CREATE TRIGGER trigger_notify_domain_alert
--     AFTER INSERT ON domain_alerts
--     FOR EACH ROW EXECUTE FUNCTION notify_domain_alert();
...
-- COMMENT ON TABLE ssl_monitors IS 'SSL/TLS certificate monitors for domain expiry tracking';
-- COMMENT ON COLUMN ssl_monitors.warning_days IS 'Days before expiry to show warning status';
-- COMMENT ON COLUMN ssl_monitors.critical_days IS 'Days before expiry to show critical status';


-- COMMENT ON TABLE monitored_domains IS 'Domain names being monitored for expiry';
-- COMMENT ON TABLE domain_check_history IS 'History of WHOIS checks for domains';
-- COMMENT ON TABLE domain_alerts IS 'Alerts generated for domain expiry warnings';

-- COMMENT ON COLUMN monitored_domains.alert_days_before IS 'Send alert when domain expires in X days';
-- COMMENT ON COLUMN monitored_domains.check_interval_hours IS 'How often to check WHOIS (in hours)';
-- COMMENT ON COLUMN monitored_domains.days_until_expiry IS 'Automatically calculated days until expiry';
...
-- ============================================
-- DOMAIN EXPIRY MONITORS TABLE
-- Tracks domain registration expiry (WHOIS-based)
-- Separate from SSL — a domain can expire even with valid SSL
-- ============================================
CREATE TABLE domain_expiry_monitors (
    id SERIAL PRIMARY KEY,
    domain VARCHAR(255) NOT NULL UNIQUE,
    friendly_name VARCHAR(100),
    check_interval INTEGER NOT NULL DEFAULT 86400,  -- 24 hours default (WHOIS rate limits)
    warning_days INTEGER NOT NULL DEFAULT 30,        -- warn if expiring within 30 days
    critical_days INTEGER NOT NULL DEFAULT 7,        -- critical if expiring within 7 days
    ...
    registrar TEXT,                  -- e.g. "GoDaddy", "Namecheap"
    registrant TEXT,                 -- domain owner
    registered_on TIMESTAMPTZ,       -- when domain was first registered
    expires_on TIMESTAMPTZ,          -- domain expiry date from WHOIS
    updated_on TIMESTAMPTZ,          -- last WHOIS update date
    days_remaining INTEGER,          -- calculated days until expiry
...
-- ============================================
-- FUNCTIONS (ALL BEFORE TRIGGERS)
-- ============================================

-- Shared updated_at — MUST be first
CREATE OR REPLACE FUNCTION update_updated_at_column()
RETURNS TRIGGER AS $$
BEGIN
    NEW.updated_at = NOW();
...
-- Domain expiry monitor notification
...
-- Helper: mark old devices offline
CREATE OR REPLACE FUNCTION mark_offline_devices(p_network_id INTEGER, p_threshold_minutes INTEGER DEFAULT 5)
RETURNS INTEGER AS $$
DECLARE affected_rows INTEGER;
BEGIN
...
-- Helper: mark old services offline
CREATE OR REPLACE FUNCTION mark_offline_services(p_threshold_minutes INTEGER DEFAULT 5)
RETURNS INTEGER AS $$
DECLARE affected_rows INTEGER;
BEGIN
...
-- Helper: get device count by network
CREATE OR REPLACE FUNCTION get_device_count_by_network(p_network_id INTEGER)
RETURNS TABLE(
...
COMMENT ON COLUMN domain_expiry_monitors.check_interval IS 'Seconds between WHOIS checks (default 86400 = 24h due to rate limits)';
COMMENT ON COLUMN domain_expiry_monitors.warning_days IS 'Days before domain expiry to trigger warning status';
COMMENT ON COLUMN domain_expiry_monitors.critical_days IS 'Days before domain expiry to trigger critical status';
COMMENT ON COLUMN domain_expiry_monitors.days_remaining IS 'Calculated days until domain registration expires';
COMMENT ON COLUMN domain_expiry_monitors.registrar IS 'Domain registrar name from WHOIS';
COMMENT ON COLUMN domain_expiry_monitors.name_servers IS 'Array of nameservers from WHOIS';
COMMENT ON COLUMN domain_expiry_monitors.status IS 'active: registered and valid, warning/critical: expiring soon, expired: past expiry date, error: WHOIS lookup failed';